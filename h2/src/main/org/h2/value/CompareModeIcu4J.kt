 ...
 */
open class CompareModeIcu4J internal constructor(name: String, strength: Int) : CompareMode(name, strength) {

    private val collator: Comparator<String> = getIcu4jCollator(name, strength)
    ...
    override fun compareString(a: String, b: String, ignoreCase: Boolean): Int {
        if (ignoreCase && getStrength() > Collator.SECONDARY) {
            ...
            if (i == null) {
                i = CompareModeIcu4J(getName(), Collator.SECONDARY)
                caseInsensitive = i
            }
    ...
    companion object {
        ...
        private fun getIcu4jCollator(name: String, strength: Int): Comparator<String> {
            try {
                var result: Comparator<String>? = null
                val collatorClass = JdbcUtils.loadUserClass<Any>(
                    "com.ibm.icu.text.Collator"
                )
                val getInstanceMethod = collatorClass.getMethod(
                    "getInstance", Locale::class.java
                )
                ...
                if (result == null) {
                    for (locale in collatorClass.getMethod(
                        "getAvailableLocales"
                    ).invoke(null) as Array<Locale>) {
                        if (compareLocaleNames(locale, name)) {
                            result = getInstanceMethod.invoke(null, locale) as Comparator<String>
                            break
                        }
