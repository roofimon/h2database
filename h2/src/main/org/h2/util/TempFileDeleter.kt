 ...
 */
class TempFileDeleter private constructor() {

    private val queue = ReferenceQueue<Any>()
    private val refMap = HashMap<PhantomReference<*>, Any>()
    ...
    fun addFile(resource: Any, monitor: Any): Reference<*> {
        ...
        IOUtils.trace(
            ...
            if (resource is String) resource else "-", monitor
        )
        val ref = PhantomReference(monitor, queue)
        refMap[ref] = resource
        deleteUnused()
        return ref
    }
     ...
     */
    @Synchronized
    fun deleteFile(ref: Reference<*>?, resource: Any?) {
        ...
        } else if (resource is AutoCloseable) {
    ...
    fun stopAutoDelete(ref: Reference<*>?, resource: Any?) {
        IOUtils.trace(
            ...
            if (resource is String) resource else "-", ref
        )
        if (ref != null) {
            val f2 = refMap.remove(ref)
            if (SysProperties.CHECK) {
                if (f2 == null || f2 != resource) {
                    throw DbException.getInternalError(
                        "f2:$f2 " + (if (f2 == null) "" else f2) + " f:$resource"
                    )
                }
            }
        }
    ...
    companion object {
        @JvmStatic
        fun getInstance(): TempFileDeleter {
