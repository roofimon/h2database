 ...
 * [DataSourceFactory.JDBC_USER], [DataSourceFactory.JDBC_PASSWORD],
 * [DataSourceFactory.JDBC_DESCRIPTION], [DataSourceFactory.JDBC_DATASOURCE_NAME],
 * [DataSourceFactory.JDBC_NETWORK_PROTOCOL], [DataSourceFactory.JDBC_URL],
 * [DataSourceFactory.JDBC_SERVER_NAME], [DataSourceFactory.JDBC_PORT_NUMBER]. The
 * following standard configuration properties are not supported:
 * [DataSourceFactory.JDBC_ROLE_NAME], [DataSourceFactory.JDBC_DATABASE_NAME],
 * [DataSourceFactory.JDBC_INITIAL_POOL_SIZE], [DataSourceFactory.JDBC_MAX_POOL_SIZE],
 * [DataSourceFactory.JDBC_MIN_POOL_SIZE], [DataSourceFactory.JDBC_MAX_IDLE_TIME],
 * [DataSourceFactory.JDBC_MAX_STATEMENTS], [DataSourceFactory.JDBC_PROPERTY_CYCLE]. Any other
 * property will be treated as an H2 specific option. If the [DataSourceFactory.JDBC_URL]
 ...
 * [DataSourceFactory.JDBC_NETWORK_PROTOCOL], [DataSourceFactory.JDBC_SERVER_NAME],
 * [DataSourceFactory.JDBC_PORT_NUMBER].
 *
 * @author Per Otterstrom
 */
class OsgiDataSourceFactory(private val driver: org.h2.Driver) : DataSourceFactory {
    ...
    override fun createDataSource(properties: Properties?): DataSource {
        // Make copy of properties
        val propertiesCopy = Properties()
        if (properties != null) {
     ...
     */
    @Throws(SQLException::class)
    override fun createConnectionPoolDataSource(
     ...
     */
    @Throws(SQLException::class)
    override fun createXADataSource(properties: Properties?): XADataSource {
        // Make copy of properties
        val propertiesCopy = Properties()
        if (properties != null) {
            propertiesCopy.putAll(properties)
     ...
     */
    @Throws(SQLException::class)
    override fun createDriver(properties: Properties?): java.sql.Driver {
    ...
    companion object {
        ...
        private fun rejectUnsupportedOptions(p: Properties) {
         ...
         */
        private fun setupH2DataSource(
            dataSource: JdbcDataSource,
            p: Properties
        ) {
            // Setting user and password
            if (p.containsKey(DataSourceFactory.JDBC_USER)) {
                dataSource.setUser(p.remove(DataSourceFactory.JDBC_USER) as String)
            }
            if (p.containsKey(DataSourceFactory.JDBC_PASSWORD)) {
                dataSource.setPassword(
                    p.remove(DataSourceFactory.JDBC_PASSWORD) as String
                )
            }
            ...
            if (p.containsKey(DataSourceFactory.JDBC_DESCRIPTION)) {
                dataSource.setDescription(
                    p.remove(DataSourceFactory.JDBC_DESCRIPTION) as String
                )
            }
            ...
            if (p.containsKey(DataSourceFactory.JDBC_URL)) {
                ...
                if (p.containsKey(DataSourceFactory.JDBC_NETWORK_PROTOCOL)) {
                    protocol = p.remove(DataSourceFactory.JDBC_NETWORK_PROTOCOL) as String
                ...
                if (p.containsKey(DataSourceFactory.JDBC_SERVER_NAME)) {
                    ...
                    connectionUrl
                        ...
                        .append("/")
                } else if (protocol == "tcp" || protocol == "ssl") {
         ...
         */
        @Throws(SQLFeatureNotSupportedException::class)
        private fun rejectPoolingOptions(p: Properties) {
        ...
        fun registerService(
            ...
            properties[DataSourceFactory.OSGI_JDBC_DRIVER_CLASS] =
                org.h2.Driver::class.java.name
            properties[DataSourceFactory.OSGI_JDBC_DRIVER_NAME] =
                "H2 JDBC Driver"
            properties[DataSourceFactory.OSGI_JDBC_DRIVER_VERSION] =
                Constants.FULL_VERSION
            properties[DataSourceFactory.OSGI_JDBC_CAPABILITY] = arrayOf(
            ...
            bundleContext.registerService(
                DataSourceFactory::class.java.name,
                OsgiDataSourceFactory(driver), properties
            )
        }
    }
}
