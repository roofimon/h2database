 ...
 */
class CipherFactory private constructor() {
    ...
    companion object {
         ...
         */
        const val KEYSTORE_PASSWORD: String =
            "h2pass"

        private const val KEYSTORE: String =
            "~/.h2.keystore"
        private const val KEYSTORE_KEY: String =
            "javax.net.ssl.keyStore"
        private const val KEYSTORE_PASSWORD_KEY: String =
         ...
         */
        @JvmStatic
        fun getBlockCipher(algorithm: String): BlockCipher {
            if ("XTEA".equals(algorithm, ignoreCase = true)) {
                return XTEA()
            } else if ("AES".equals(algorithm, ignoreCase = true)) {
                return AES()
            } else if ("FOG".equals(algorithm, ignoreCase = true)) {
                return Fog()
            }
        ...
        fun createSocket(address: InetAddress, port: Int): Socket {
            setKeystore()
            val f = SSLSocketFactory.getDefault() as SSLSocketFactory
            val secureSocket = f.createSocket() as SSLSocket
            secureSocket.connect(
                InetSocketAddress(address, port),
                SysProperties.SOCKET_CONNECT_TIMEOUT
            )
            secureSocket.enabledProtocols =
                disableSSL(secureSocket.enabledProtocols)
            return secureSocket
        }
        ...
        @JvmStatic
        @Throws(IOException::class)
        fun createServerSocket(port: Int, bindAddress: InetAddress?): ServerSocket {
            setKeystore()
            val f = SSLServerSocketFactory.getDefault()
            val secureSocket: SSLServerSocket
            if (bindAddress == null) {
                secureSocket = f.createServerSocket(port) as SSLServerSocket
            } else {
                secureSocket = f.createServerSocket(port, 0, bindAddress) as SSLServerSocket
            }
            secureSocket.enabledProtocols =
                disableSSL(secureSocket.enabledProtocols)
            return secureSocket
        ...
        @Throws(IOException::class)
        private fun getKeyStoreBytes(store: KeyStore, password: String): ByteArray {
        ...
        @JvmStatic
        @Throws(IOException::class)
        fun getKeyStore(password: String): KeyStore {
            try {
        ...
        private fun setKeystore() {
            ...
            if (p.getProperty(KEYSTORE_KEY) == null) {
                val fileName = KEYSTORE
                val data = getKeyStoreBytes(
                    getKeyStore(KEYSTORE_PASSWORD), KEYSTORE_PASSWORD
                )
                var needWrite = true
                if (FileUtils.exists(fileName) && FileUtils.size(fileName) == data.size.toLong()) {
                    // don't need to overwrite the file if it did not change
                    val fin = FileUtils.newInputStream(fileName)
                    val now = IOUtils.readBytesAndClose(fin, 0)
                ...
                if (needWrite) {
                    try {
                        val out = FileUtils.newOutputStream(fileName, false)
                        out.write(data)
                        out.close()
                    } catch (e: Exception) {
                        throw DataUtils.convertToIOException(e)
                ...
                val absolutePath = FileUtils.toRealPath(fileName)
        ...
        private fun disableSSL(enabled: Array<String>): Array<String> {
